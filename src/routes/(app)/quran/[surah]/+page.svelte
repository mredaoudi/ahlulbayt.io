<script>
    ...
    onMount(() => {
        ...
        if (hash) {
            ...
            if (el) {
                el.classList.add("bg-slate-100", "transition-all");
...
<div class="flex flex-col items-center justify-center text-slate-700">
    <div class="font-bold"><span style="font-family: 'Scheherazade New'">{surah.ar}</span> - {surah._a}</div>
    <div>{surah.en}</div>
...
<div class="mt-4 flex flex-row justify-between text-sm text-slate-500">
    ...
    <div class="flex items-center hover:underline">
        ...
        <a href={`/quran/${previousSurah._n}`}><span style="font-family: 'Scheherazade New'">{previousSurah.ar}</span> - {previousSurah._a}</a>
    </div>
    {:else}
        <span></span>
    {/if}
    {#if nextSurah}
        <div class="flex items-center hover:underline">
            <a href={`/quran/${nextSurah._n}`}><span style="font-family: 'Scheherazade New'">{nextSurah.ar}</span> - {nextSurah._a}</a>
...
>
    <div
        class="absolute -top-6 left-1/2 transform -translate-x-1/2 border px-10 py-2 rounded text-lg bg-zinc-50 text-slate-700 {isTawbah && searchTextState.value === '' ? 'hidden' : ''}"
    ...
    {#each filteredVerses as verse, i}
        <div
            id="v{verse._n}"
            class="flex flex-col py-6 px-8 {i === 0 ? 'pt-8' : ''} gap-6 {i === filteredVerses.length - 1 ? '' : 'border-b'} border-slate-200 relative"
        >
            <div
            ...
            <div class="font-light">{@html verse.en}</div>
        </div>
    {/each}
</div>
